...
- **Product catalog media tiers** (under `public/assets/**`):
  - Registry paths in [`src/constants/products.ts`](src/constants/products.ts) stay canonical (`.jpg` / `.png`).
  - **Listing hover:** `.listing.webp` (1280px) via `getListingDemoWorkPreviewPath()` — generated by `pnpm optimize:product-images`.
  - **Detail gallery / panorama / lightbox:** `.detail.webp` (1920px) via `resolveDetailGalleryImagePath()` — generated by `pnpm generate:detail-media`.
  - **Large originals** are moved to `public/assets/_archive/originals/` by `pnpm archive:product-media` after sidecars exist; never reference `_archive/` from `src/`.
  - **Size mirrors:** `pnpm sync:product-size-assets` copies `60X120`/`80X80` sources to `100X100` & `120X120`; `60X120` products that also list `80×80cm` get an extra mirror into `80X80/`.
  - Audit: `pnpm audit:product-media --strict` before deploy.
