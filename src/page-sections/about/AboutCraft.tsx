...
function KilnSketch({ captionOpacity }: SketchProps) {
	...
	return (
		<motion.svg viewBox="0 0 200 120" fill="none" className="w-full" aria-hidden="true" style={{ opacity: sketchOpacity }}>
			<rect
				x="30" y="15" width="140" height="80"
				stroke={T.color} strokeWidth={T.strokeRegular} strokeOpacity={T.opacityAnnotation}
			...
			<path
				...
				stroke={T.color} strokeWidth={T.strokeHairline} strokeOpacity={T.opacitySketch}
			...
			<text
				...
				fill={T.color} fillOpacity={T.opacitySketch}
			>
				1200°C
			</text>
		</motion.svg>
...
function TileSketch({ captionOpacity }: SketchProps) {
	...
	return (
		<motion.svg viewBox="0 0 200 120" fill="none" className="w-full" aria-hidden="true" style={{ opacity: sketchOpacity }}>
			<rect
				x="20" y="30" width="160" height="50"
				stroke={T.color} strokeWidth={T.strokeRegular} strokeOpacity={T.opacityAnnotation}
				vectorEffect="non-scaling-stroke"
			/>
			<rect
				x="20" y="22" width="160" height="10"
				stroke={T.color} strokeWidth={T.strokeHairline} strokeOpacity={T.opacityAnnotation}
				strokeDasharray={T.dashSketch} vectorEffect="non-scaling-stroke"
			/>
			<path
				d="M 40 22 L 40 80 M 80 22 L 80 80 M 120 22 L 120 80 M 160 22 L 160 80"
				stroke={T.color} strokeWidth={T.strokeHairline} strokeOpacity="0.12"
				strokeDasharray={T.dashSketch} vectorEffect="non-scaling-stroke"
			/>
			<path
	...
	return (
		<motion.svg viewBox="0 0 200 120" fill="none" className="w-full" aria-hidden="true" style={{ opacity: sketchOpacity }}>
			...
			<path
				...
				stroke={T.color} strokeWidth={T.strokeHairline} strokeOpacity={T.opacityAnnotation}
				strokeDasharray={T.dashDimension} vectorEffect="non-scaling-stroke"
			/>
			<rect
				x="10" y="60" width="180" height="16"
				stroke={T.color} strokeWidth={T.strokeHairline} strokeOpacity={T.opacitySketch}
				strokeDasharray={T.dashSketch} vectorEffect="non-scaling-stroke"
			/>
			<path
			...
			<path
				...
				stroke={T.color} strokeWidth={T.strokeHairline} strokeOpacity="0.15"
				vectorEffect="non-scaling-stroke"
			/>
			<path
...
export function AboutCraft() {
	...
	const captionY = [
		...
		useTransform(scrollYProgress, [0.30, 0.40], [20, 0]),
				...
				<div className="relative flex min-h-0 flex-1 flex-col bg-[#071A2B] px-6 py-6 sm:px-8 sm:py-8 lg:h-full lg:w-1/2 lg:px-14 lg:py-20">
					{CRAFT_BEATS.map((beat, index) => (
						<motion.div
							...
							className="pointer-events-none absolute right-6 top-5 text-right sm:right-8 sm:top-6 lg:right-12 lg:top-14"
							aria-hidden="true"
						>
					...
					<div className="relative flex min-h-0 flex-1 flex-col justify-center overflow-y-auto lg:overflow-visible">
						{CRAFT_BEATS.map((beat, index) => {
							...
							return (
								...
								>
									<span
										className="block font-serif text-[32px] font-light leading-none text-transparent sm:text-[38px] lg:text-[48px]"
									...
									<span className="-mt-1 block font-sans text-label tracking-widest text-[#D4B886]/65 uppercase">
									...
									<Text
										variant="h2"
										as="h2"
										className="mt-3 font-serif text-[28px] font-light italic leading-tight text-[#F4F4F6] sm:mt-4 sm:text-display-lg lg:mt-5"
									>
									...
									<Text
										variant="body"
										className="mt-3 max-w-none leading-relaxed text-[#F4F4F6]/60 sm:mt-4 sm:max-w-sm sm:text-body-lg"
									>
						...
						{CRAFT_BEATS.map((beat, index) => (
							...
							>
								<motion.div
									...
									style={{ scaleX: segmentProgress[index], transformOrigin: "left" }}
								/>
							</div>
						))}
					</div>
