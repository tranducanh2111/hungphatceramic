...
function ChandelierMarkup({ totalCards }: ChandelierMarkupProps) {
	...
	return (
		<>
			...
			<ellipse cx="0" cy="-242" rx="14" ry="4" stroke="#D4B886" strokeWidth="0.7" strokeOpacity="0.4" fill="none" />
			...
			<ellipse cx="0" cy="-130" rx="40" ry="9" stroke="#D4B886" strokeWidth="0.7" strokeOpacity="0.32" fill="none" />
			...
			{[0, 72, 144, 216, 288].map((degree) => {
				...
				return (
					<g key={degree}>
						<line x1={hangerX} y1={hangerY} x2={hangerX} y2={hangerY + 14} stroke="#D4B886" strokeWidth="0.4" strokeOpacity="0.25" />
						<circle cx={hangerX} cy={hangerY + 16} r="1.8" fill="#D4B886" fillOpacity="0.32" />
					</g>
				);
			})}

			<ellipse cx="0" cy="0" rx="20" ry="5.5" stroke="#D4B886" strokeWidth="1" strokeOpacity="0.55" fill="none" />
			<ellipse cx="0" cy="-1.5" rx="12" ry="3.5" stroke="#D4B886" strokeWidth="0.55" strokeOpacity="0.3" fill="none" />
			...
			{armAngles.map((degree, index) => {
				...
				return (
					<g key={index}>
						...
						/>
						<circle cx={tipX * 0.88} cy={tipY + 25} r="2.2" fill="#D4B886" fillOpacity={Math.max(0.12, opacity + 0.05)} />
						<circle cx={tipX * 0.88} cy={tipY + 30} r="1.3" fill="#D4B886" fillOpacity={Math.max(0.08, opacity - 0.05)} />
					</g>
				);
			})}
			...
			<ellipse cx="0" cy="78" rx="26" ry="6" stroke="#D4B886" strokeWidth="0.55" strokeOpacity="0.22" fill="none" />
			{[0, 60, 120, 180, 240, 300].map((degree) => {
				const radian = (degree * Math.PI) / 180;
				return (
					<line
			...
			<ellipse cx="0" cy="178" rx="4.5" ry="7" stroke="#D4B886" strokeWidth="0.7" strokeOpacity="0.35" fill="none" />
	...
	return (
		...
		>
			<div className="relative h-full w-full" style={{ transformStyle: "preserve-3d" }}>
				...
				<div className={cn(SPIRAL_CARD_FACE_CLIP_CLASS, "[transform:rotateY(180deg)_translateZ(0.1px)]")}>
					<div className="h-full w-full [transform:rotateY(180deg)]" style={{ transformStyle: "flat" }}>
						<ProjectCardBack
							project={project}
							index={index}
							translationNamespace={translationNamespace}
		...
		>
			...
			>
				<defs>
					...
					<linearGradient id="shaft-fade" x1="0" y1="-260" x2="0" y2="260" gradientUnits="userSpaceOnUse">
...
function ProjectsSpiralStage({
	...
	return (
		...
		>
			...
			<motion.div style={{ rotateY, rotateX, transformStyle: "preserve-3d", willChange: isInView ? "transform" : "auto" }}>
				{FEATURED_PROJECTS.map((project, index) => (
					<ProjectSpiralCard key={project.id} project={project} index={index} geometry={geometry} />
				))}
			</motion.div>
		</motion.div>
	);
	...
	return (
		<div className="mt-10 grid gap-6 sm:grid-cols-2">
			{FEATURED_PROJECTS.map((project, index) => (
				...
				>
					<ProjectCardVisual project={project} index={index} translationNamespace={`items.${project.id}`} />
				</motion.div>
			))}
		</div>
	);
...
export function ProjectsSpiralExperience({ geometry }: ProjectsSpiralExperienceProps) {
	...
	const translateY = useSpring(useTransform(scrollYProgress, [0, 1], [totalHeight / 2, -totalHeight / 2]), SPRING_CONFIG);

	return (
		<div ref={spiralZoneRef} className="relative hidden md:block">
			<div
				className="sticky top-0 h-screen overflow-hidden"
				style={{ perspective: "1400px", perspectiveOrigin: "50% 45%", transformStyle: "preserve-3d" }}
			>
				<ProjectsSpiralStage
					geometry={geometry}
